"""SpaceMouse teleoperation for robosuite Panda + PandaGripper.

Six SpaceMouse axes are interpreted relative to the configured primary camera,
then drive the Panda end effector through world-frame ``OSC_POSE``.  The
configured left button closes the official one-DoF parallel
gripper and the right button opens it.  ``R`` resets; ``Q`` and Ctrl+C safely
send a zero action and release resources.
"""
